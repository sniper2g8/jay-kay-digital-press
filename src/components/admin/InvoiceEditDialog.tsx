...
export const InvoiceEditDialog = ({ isOpen, onClose, onInvoiceUpdated, invoice }: InvoiceEditDialogProps) => {
  ...
  useEffect(() => {
    if (selectedJob && !invoice) { // Only auto-populate for new invoices
      const job = jobs.find(j => j.id.toString() === selectedJob);
      if (job) {
        setItems([{
          description: job.description || job.title || "Printing Service",
          quantity: job.quantity || 1,
          unit_price: job.final_price || job.quoted_price || 0,
          total_price: (job.final_price || job.quoted_price || 0) * (job.quantity || 1)
        }]);
      }
    }
  }, [selectedJob, jobs, invoice]);
  ...
  const loadInvoiceData = async () => {
    ...
    try {
      ...
      setSelectedJob(invoice.job_id?.toString() || "");
  ...
  const fetchCustomerJobs = async (customerId: string) => {
    const { data, error } = await supabase
      .from('jobs')
      .select('id, title, description, customer_uuid, final_price, quoted_price, tracking_code, quantity')
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Select value={status} onValueChange={(value: "draft" | "sent" | "paid" | "overdue" | "cancelled") => setStatus(value)}>
                <SelectTrigger>
